...
(*avoid conflict module name after open Js*)
module A = Array
...
external int8Array : (int -> int8Array t) = "Int8Array" [@@bs.new]
    ...
    (* original functions*)
    let set (typedArray : ('a, 'b) typedArray Js.t) index (value : 'a) = A.set (Util.cast (typedArray :> < > Js.t)) index value

    let get (typedArray : ('a, 'b) typedArray Js.t) index : 'a = A.get (Util.cast (typedArray :> < > Js.t)) index