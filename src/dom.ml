...
let get_documentPosition (documentPosition : documentPosition) =
  let ans = ref [] in
  if has documentPosition disconnected then
    ans := `DISCONNECTED :: !ans;
  if has documentPosition preceding then
    ans := `PRECEDING :: !ans;
  if has documentPosition following then
    ans := `FOLLOWING :: !ans;
  if has documentPosition contains then
    ans := `CONTAINS :: !ans;
  if has documentPosition contained_by then
    ans := `CONTAINED_BY :: !ans;
  if has documentPosition implementation_specific then
    ans := `IMPLEMENTATION_SPECIFIC :: !ans;
  !ans